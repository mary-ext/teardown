...
describe('hoist', () => {
	describe('basic hoisting', () => {
		...
		it('hoists deep dependency chain to root', () => {
			...
			expect(countLevels(hoistedToPaths(result))).toEqual({ root: 4, nested: 0 });
		});
		...
		it('handles package with no dependencies', () => {
			...
			expect(countLevels(hoistedToPaths(result))).toEqual({ root: 1, nested: 0 });
		});
	...
	describe('cyclic dependencies', () => {
		it('handles simple cycle between two packages', () => {
			...
			expect(countLevels(hoistedToPaths(result))).toEqual({ root: 2, nested: 0 });
		});
		...
		it('handles longer cycle (A -> B -> C -> A)', () => {
			...
			expect(countLevels(hoistedToPaths(result))).toEqual({ root: 3, nested: 0 });
		});
